...
2. Download Project
	Download the SISv5n2.zip from the course web. Unzip the file, and put it under any path you want.

	Note: The difference between SISv5n1 and SISv5n2 is tempcomponentsource.jar was replaced.
...
5.1. Initialize:
		a). Go to SISv5/NewSISServer/: 
			i) In terminal run javac *.java to compile all of the java files
			ii) In terminal run java SISServer
			iii) In a separate terminal run java -jar PrjRemote.jar this will open a GUI window
			iv) In the GUI hit Connection & Scope in the dropdown you will see a connect button hit it.  This will connect it to the sis terminal server you have running.
			v) From here on you can load in messages in the messages window.  

			For more information see the ReadMe_V5.txt
			
				
5.2. Running SIS Testbed on macOS X

Since SIS Testbed is written in Java, it is possible to run the testbed in macOS. However, the scripts generated by the testbed are **Windows batch files**. We can convert these batch files to **bash scripts**, which macOS X supports natively.

We do not need an automatic batch-to-bash converter, as the batch files generated by the SIS Testbed are very simple. A batch file generated by the testbed usually consists of just two lines: 

- Compile the Java source file
- Run the compiled Java class file

Thus we will simply convert the two lines to bash statements manually. 

Do the conversion just once for each *.bat file, and then you can delete them and use only the *.sh files.


Below are some common examples of the conversion:
...
You should create a file named runserver.sh with the following content:
...
EXAMPLE 2 (runInitializer)

For runInitializer.bat:
...
You should create a file named runInitializer.sh with the following content:
...
You should create a file named runGUI.sh with the following content:
...
As you can see, the conversion is straightforward. The only thing you need to pay close attention to how Windows and Mac differ when concatenating paths. In Windows, paths are joined by ";", while on Mac, paths are joined by ":".
...
IF RUNNING ON WINDOWS, AND CHANGES
TO YOUR CreateComponent.java DO NOT
APPEAR IN YOUR COMPONENT:

1) Delete 'CreateComponent.class'
(or whatever you named your file,
should be in SISv5 -> Components -> YourComponent)

2) Run 'javac *.java' from the SISv5
-> Components -> YourComponent directory

3) Now launch runserver.bat,
runInitializer.bat, and
runYouComponent.bat as usual
